...
from mrmax.operators.LinearOperator import LinearOperator, LinearOperatorSum
...
class LinearOperatorMatrix(Operator):
    ...
    A matrix of linear operators, where each element is a `~mrmax.operators.LinearOperator`.
    ...
    If indexing returns a single element, it is returned as a `~mrmax.operators.LinearOperator`.

    Basic arithmetic operations are supported with `~mrmax.operators.LinearOperator` and Tensors.
    """

    _operators: list[list[LinearOperator]]
    _shape: tuple[int, int]

    def __init__(self, operators: Sequence[Sequence[LinearOperator]]) -> None:
        """Initialize Linear Operator Matrix.

        Create a matrix of `~mrmax.operators.LinearOperator` from a sequence of rows, where each row is a sequence
        of `~mrmax.operators.LinearOperator` that represent the columns of the matrix.

        Parameters
        ----------
        operators
            A sequence of rows, which are sequences of `~mrmax.operators.LinearOperator`.
        """
        ...
        self._operators = cast(  # cast because ModuleList is not recognized as a list
            list[list[LinearOperator]], eqx.ModuleList(eqx.ModuleList(row) for row in operators)
        )
    ...
    def forward(self, *x: Array) -> tuple[Array, ...]:
        ...
        if len(x) != self.shape[1]:
            raise ValueError('Number of input tensors does not match number of columns.')
        return tuple(
            reduce(operator.add, (op(xi)[0] for op, xi in zip(row, x, strict=True))) for row in self._operators
        )

    def __getitem__(self, idx: _IdxType) -> LinearOperatorMatrix | LinearOperator:
        ...
        Returns
        -------
            Subset `LinearOperatorMatrix` or `~mrmax.operators.LinearOperator`.
        """
    ...
    def __add__(self, other: LinearOperatorMatrix | LinearOperator | Array) -> LinearOperatorMatrix:
        """Addition."""
        ...
        elif isinstance(other, (LinearOperator, Array)):
    ...
    def __radd__(self, other: LinearOperatorMatrix | LinearOperator | Array) -> LinearOperatorMatrix:
        """Right addition."""
        return self.__add__(other)

    def __mul__(self, other: Array | Sequence[Array | complex] | complex) -> LinearOperatorMatrix:
        """LinearOperatorMatrix*Tensor multiplication.

        Example: ([A,B]*c)(x) = [A*c, B*c](x) = A(c*x) + B(c*x)
        """
        if isinstance(other, (Array, complex, float, int)):
            other_: Sequence[Array | complex] = (other,) * self.shape[1]
    ...
    def __rmul__(self, other: Array | Sequence[Array] | complex) -> LinearOperatorMatrix:
        """Tensor*LinearOperatorMatrix multiplication.

        Example: (c*[A,B])(x) = [c*A, c*B](x) = c*A(x) + c*B(x)
        """
        if isinstance(other, (Array, complex, float, int)):
            other_: Sequence[Array | complex] = (other,) * self.shape[0]
    ...
    def __rmatmul__(self, other: LinearOperator) -> LinearOperatorMatrix:
        """Composition of operators."""
        operators = [[op @ other for op in row] for row in self._operators]
        return self.__class__(operators)

    def __matmul__(self, other: LinearOperator | LinearOperatorMatrix) -> LinearOperatorMatrix:
        """Composition of operators."""
    ...
    @property
    def H(self) -> LinearOperatorMatrix:  # noqa N802
    ...
    @classmethod
    def from_diagonal(cls, *operators: LinearOperator) -> LinearOperatorMatrix:
        ...
        ----------
        operators
            Sequence of `~mrmax.operators.LinearOperator` to be placed on the diagonal.
        """
    ...
    def operator_norm(
        ...
        *initial_value: Array,
        ...
        def _singlenorm(op: LinearOperator, initial_value: Array) -> Array:
        ...
        norm = jnp.sqrt(jnp.sum(jnp.square(norms), axis=-2)).max(axis=-1, keepdims=True)
        return norm

    def __or__(self, other: LinearOperator | LinearOperatorMatrix) -> LinearOperatorMatrix:
        """Vertical stacking."""
        if isinstance(other, LinearOperator):
            ...
            if (rows_self := self.shape[0]) != (rows_other := other.shape[0]):
                raise ValueError(
                    f'Shape mismatch in vertical stacking: cannot stack matrices with {rows_self} and {rows_other} rows.'
                )
            operators = [[*self_row, *other_row] for self_row, other_row in zip(self, other, strict=True)]
            return self.__class__(operators)

    def __ror__(self, other: LinearOperator) -> LinearOperatorMatrix:
        """Vertical stacking."""
        if rows := self.shape[0] > 1:
            raise ValueError(
                f'Shape mismatch in vertical stacking: cannot stack LinearOperator and matrix with {rows} rows.'
    ...
    def __and__(self, other: LinearOperator | LinearOperatorMatrix) -> LinearOperatorMatrix:
        """Horizontal stacking."""
        if isinstance(other, LinearOperator):
    ...
    def __rand__(self, other: LinearOperator) -> LinearOperatorMatrix:
        """Horizontal stacking."""
        if cols := self.shape[1] > 1:
            raise ValueError(
                f'Shape mismatch in horizontal stacking: cannot stack LinearOperator and matrix with {cols} columns.'
