...
import mrmax.operators
from mrmax.operators.Operator import Operator
from mrmax.utils.typing import endomorph
...
class EndomorphOperator(Operator[Unpack[tuple[torch.Tensor, ...]], tuple[torch.Tensor, ...]]):
    ...
    def __matmul__(
        ...
        if isinstance(other, mrmax.operators.MultiIdentityOp):
            return self
        elif isinstance(self, mrmax.operators.MultiIdentityOp):
            return other
