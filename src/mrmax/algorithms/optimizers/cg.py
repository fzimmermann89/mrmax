...
def cg(
    ...
    # initial residual
    residual = right_hand_side - operator(initial_value)[0] if initial_value is not None else jnp.array(right_hand_side)

    # initialize conjugate vector
    conjugate_vector = jnp.array(residual)

    # assign starting value to the solution
    solution = jnp.array(initial_value) if initial_value is not None else jnp.array(right_hand_side)

    # for the case where the residual is exactly zero
    if jnp.vdot(residual.flatten(), residual.flatten()) == 0:
        return solution

    # dummy value. new value will be set in loop before first usage
    residual_norm_squared_previous: jnp.ndarray | None = None

    for iteration in range(max_iterations):
        # calculate the square norm of the residual
        residual_flat = residual.flatten()
        residual_norm_squared = jnp.vdot(residual_flat, residual_flat).real
        ...
        alpha = residual_norm_squared / (jnp.vdot(conjugate_vector.flatten(), operator_conjugate_vector.flatten()))
