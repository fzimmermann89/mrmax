...
from mrmax.operators.functionals import ZeroFunctional
...
class PDHGStatus(OptimizerStatus):
    ...
    objective: Callable[[*tuple[jnp.ndarray, ...]], jnp.ndarray]
    dual_stepsize: float | jnp.ndarray
    primal_stepsize: float | jnp.ndarray
    relaxation: float | jnp.ndarray
    duals: Sequence[jnp.ndarray]
    relaxed: Sequence[jnp.ndarray]


def _norm_squared(*values: jnp.ndarray) -> jnp.ndarray:
    """Calculate the squared L2 norm of the stack of arrays."""
    return sum([jnp.vdot(value.flatten(), value.flatten()).real for value in values], start=jnp.array(0.0))
...
def pdhg(
    ...
    initial_values: Sequence[jnp.ndarray],
    max_iterations: int = 32,
    tolerance: float = 0,
    primal_stepsize: float | None = None,
    dual_stepsize: float | None = None,
    relaxation: float = 1.0,
    initial_relaxed: Sequence[jnp.ndarray] | None = None,
    initial_duals: Sequence[jnp.ndarray] | None = None,
    ...
    The operator can be supplied as a `~mrmax.operators.LinearOperator` or as a
    :math:`m\times n` -`~mrmax.operators.LinearOperatorMatrix`, :math:`f` and :math:`g` can either be single functionals
    or `~mrmax.operators.ProximableFunctionalSeparableSum` of m, or n, respectively, functionals.
    ...
    if primal_stepsize is None or dual_stepsize is None:
        ...
        operator_norm = operator_matrix.operator_norm(*[jax.random.normal(key, v.shape) for v in initial_values])
    ...
    for i in range(max_iterations):
        ...
        primals_relaxed = tuple(
            jnp.lerp(primal, primal_new, relaxation) for primal, primal_new in zip(primals, primals_new, strict=False)
        )
