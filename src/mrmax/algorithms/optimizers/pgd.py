...
from mrmax.operators import ProximableFunctionalSeparableSum
...
class PGDStatus(OptimizerStatus):
    ...
    stepsize: float | jnp.ndarray
    objective: Callable[..., jnp.ndarray]


def pgd(
    f: Operator[jnp.ndarray, tuple[jnp.ndarray]] | Operator[Unpack[tuple[jnp.ndarray, ...]], tuple[jnp.ndarray]],
    g: ProximableFunctional | ProximableFunctionalSeparableSum,
    initial_value: jnp.ndarray | tuple[jnp.ndarray, ...],
    stepsize: float = 1.0,
    ...
    Example:
        ...
        In this case, :math:`||F^T F|| = 1`. ::
                kspace_data = jnp.random.normal(key, (3, 10, 10), dtype=jnp.complex64)
                fft = FastFourierOp()
                ...
                initial_value = jnp.ones((3,10,10))
    ...
    if isinstance(initial_value, jnp.ndarray):
        initial_values: tuple[jnp.ndarray, ...] = (initial_value,)
    else:
    ...
    y: tuple[jnp.ndarray, ...] = initial_values
    gradient: tuple[jnp.ndarray, ...]
    f_y: jnp.ndarray
    t_old = 1.0
    grad_and_value_f = jax.grad(lambda *x: f(*x)[0], has_aux=True)

    # Initialize x to avoid "possibly unbound" error
    x = initial_values
    ...
    for iteration in range(max_iterations):
        while stepsize > 1e-30:
            ...
            quadratic_approx = (
                f_y
                + 1 / (2 * stepsize) * sum(jnp.abs(di).sum() ** 2 for di in difference)
                + sum(jnp.vdot(gi.flatten(), di.flatten()).real for gi, di in zip(gradient, difference, strict=True))
            )
