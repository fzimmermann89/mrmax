...
# %% [markdown]
# We use the `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction` class to reconstruct images by solving
...
# %% [markdown]
# ## Using `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction`
# First, we demonstrate the use of `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction`, before we
...
# `~mrmax.algorithms.reconstruction.DirectReconstruction` class.
# See also <project:direct_reconstruction.ipynb>.

# %%
direct_reconstruction = mrmax.algorithms.reconstruction.DirectReconstruction(kdata)
img_direct = direct_reconstruction(kdata)

# %% [markdown]
# ### Setting up the iterative SENSE reconstruction
# Now let's use the `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction` class to reconstruct the image
...
#
# ```{note}
# When settings up the reconstruction can also just provide the `~mrmax.data.KData` and let
#  `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction` figure
# out the Fourier operator, estimate the coil sensitivity maps, and choose a density weighting.\
# We can also provide `~mrmax.data.KData` and some information, such as the sensitivity maps.
# In that case, the reconstruction will fill in the missing information based on the `~mrmax.data.KData` object.
# ```

# %%
iterative_sense_reconstruction = mrmax.algorithms.reconstruction.IterativeSENSEReconstruction(
...
# and obtain the reconstructed image as `~mrmax.data.IData` object.
# %%
...
# `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction` does when initialized with only an `~mrmax.data.KData`
...
# Voronoi tessellation of the trajectory to calculate the `~mrmax.data.DcfData`.
#
# ```{note}
# Using a weighted loss in iterative SENSE is not necessary, and there has been some discussion about
# the benefits and drawbacks. Currently, the iterative SENSE reconstruction in mrmax uses a weighted loss.
...
# We need `~mrmax.operators.FourierOp` and `~mrmax.operators.SensitivityOp` operators to set up the acquisition model
...
# %%
img_coilwise = mrmax.data.IData.from_tensor_and_kheader(*fourier_operator.H(*dcf_operator(kdata.data)), kdata.header)
csm_data = mrmax.data.CsmData.from_idata_walsh(img_coilwise)
...
# %%
img_manual = mrmax.algorithms.optimizers.cg(
...
# Both versions, the one using the `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction` class
