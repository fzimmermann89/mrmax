...
# Here we use the `~mrmax.algorithms.reconstruction.DirectReconstruction` class to perform a basic reconstruction of
# 2D radial data.
# A *direct* reconstruction uses the density compensated adjoint of the acquisition operator to obtain the images.

# %% [markdown]
# ## Using `~mrmax.algorithms.reconstruction.DirectReconstruction`
# We use the `~mrmax.algorithms.reconstruction.DirectReconstruction` class to reconstruct images from 2D radial data.
# `~mrmax.algorithms.reconstruction.DirectReconstruction` estimates sensitivity maps, density compensation factors, etc.
...
# This can be done by passing a `~mrmax.data.traj_calculators.KTrajectoryIsmrmrd` object to
# `~mrmax.data.KData.from_file` when loading creating the `~mrmax.data.KData`.
...
trajectory_calculator = mrmax.data.traj_calculators.KTrajectoryIsmrmrd()
kdata = mrmax.data.KData.from_file(data_folder / 'radial2D_402spokes_golden_angle_with_traj.h5', trajectory_calculator)

# %% [markdown]
### Setup the DirectReconstruction instance
# We create a `~mrmax.algorithms.reconstruction.DirectReconstruction` and supply ``kdata``.
# `~mrmax.algorithms.reconstruction.DirectReconstruction` uses the information in ``kdata`` to
...
# We now got in `~mrmax.data.IData` object containing a header and the image tensor.
...
# %% [markdown]
# ## Behind the scenes
# We now peek behind the scenes to see what happens in the `~mrmax.algorithms.reconstruction.DirectReconstruction`
...
# We use a Voronoi tessellation of the trajectory to calculate the `~mrmax.data.DcfData` and obtain
# a `~mrmax.operators.DensityCompensationOp` operator.

# %%
dcf_operator = mrmax.data.DcfData.from_traj_voronoi(kdata.traj).as_operator()

# %% [markdown]
# ### Setup Fourier Operator
# Next, we create the Fourier operator. We can just pass the ``kdata`` object to the constructor of the
# `~mrmax.operators.FourierOp`, and the trajectory and header information is used to create the operator. We want the
# to use the adjoint density compensated Fourier operator, so we perform a composition with ``dcf_operator``
# and use the `~mrmax.operators.FourierOp.H` property of the operator to obtain its adjoint.

# %%
fourier_operator = dcf_operator @ mrmax.operators.FourierOp.from_kdata(kdata)
adjoint_operator = fourier_operator.H

# %% [markdown]
# ### Calculate coil sensitivity maps
# Coil sensitivity maps are calculated using the walsh method (See `~mrmax.data.CsmData` for other available methods).
# We first need to calculate the coil-wise images, which are then used to calculate the coil sensitivity maps.

# %%
img_coilwise = mrmax.data.IData.from_tensor_and_kheader(*adjoint_operator(kdata.data), kdata.header)
csm_operator = mrmax.data.CsmData.from_idata_walsh(img_coilwise).as_operator()

# %% [markdown]
# ### Perform Direct Reconstruction
# Finally, the direct reconstruction is performed and an `~mrmax.data.IData` object with the reconstructed
...
img_manual = mrmax.data.IData.from_tensor_and_kheader(*adjoint_operator(kdata.data), kdata.header)
...
# %%
fourier_operator = mrmax.operators.FourierOp(
    recon_matrix=kdata.header.recon_matrix,
    encoding_matrix=kdata.header.encoding_matrix,
    traj=kdata.traj,
)
# %% [markdown]
# We can call one of the algorithms in `mrmax.algorithms.dcf` to calculate the density compensation factors.

# %%
kykx = torch.stack((kdata.traj.ky[0, 0], kdata.traj.kx[0, 0]))
dcf_tensor = mrmax.algorithms.dcf.dcf_2d3d_voronoi(kykx)

# %% [markdown]
# We use these DCFs to weight the k-space data before performing the adjoint Fourier transform. We can also call
# `~mrmax.operators.FourierOp.adjoint` on the Fourier operator instead of obtaining an adjoint operator.

# %%
(img_tensor_coilwise,) = fourier_operator.adjoint(dcf_tensor * kdata.data)

# %% [markdown]
# Next, we calculate the coil sensitivity maps by using one of the algorithms in `mrmax.algorithms.csm` and set
# up a `~mrmax.operators.SensitivityOp` operator.

# %%
csm_data = mrmax.algorithms.csm.walsh(img_tensor_coilwise[0], smoothing_width=5)
csm_operator = mrmax.operators.SensitivityOp(csm_data)
...
img_more_manual = mrmax.data.IData.from_tensor_and_kheader(img_tensor_coilcombined, kdata.header)
