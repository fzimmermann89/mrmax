...
# `~mrmax.algorithms.optimizers.pdhg`.
...
kdata_402spokes = mrmax.data.KData.from_file(
    data_folder / 'radial2D_402spokes_golden_angle_with_traj.h5', mrmax.data.traj_calculators.KTrajectoryIsmrmrd()
)
kdata_24spokes = mrmax.data.KData.from_file(
    data_folder / 'radial2D_24spokes_golden_angle_with_traj.h5', mrmax.data.traj_calculators.KTrajectoryIsmrmrd()
)

# %% [markdown]
# ### Comparison reconstructions
# Before running the TV-minimization reconstruction, we first run a direct (adjoint) reconstruction
# using `~mrmax.algorithms.reconstruction.DirectReconstruction` (see <project:direct_reconstruction.ipynb>)
# of both the 24 spokes and 402 spokes data to have a reference for comparison.

# %%
direct_reconstruction_402 = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_402spokes)
direct_reconstruction_24 = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_24spokes)
...
# %%
sense_reconstruction = mrmax.algorithms.reconstruction.IterativeSENSEReconstruction(
    kdata_24spokes,
    n_iterations=8,
...
# `~mrmax.operators.FourierOp` and a `~mrmax.operators.SensitivityOp`, which applies the coil sensitivity maps
# (CSM) to the image. We reuse the CSMs estimated in the direct reconstruction.

# %%
fourier_operator = mrmax.operators.FourierOp.from_kdata(kdata_24spokes)
...
f_1 = 0.5 * mrmax.operators.functionals.L2NormSquared(target=kdata_24spokes.data, divide_by_n=True)
f_2 = regularization_lambda * mrmax.operators.functionals.L1NormViewAsReal(divide_by_n=True)
f = mrmax.operators.ProximableFunctionalSeparableSum(f_1, f_2)

# %% [markdown]
# #### $K(x) = [A, \nabla]^T$
#
#   where $\nabla$ is the finite difference operator that computes the directional derivatives along the last two
#   dimensions (y,x), implemented as `~mrmax.operators.FiniteDifferenceOp`, and
#  `~mrmax.operators.LinearOperatorMatrix` can be used to stack the operators.

# %%
nabla = mrmax.operators.FiniteDifferenceOp(dim=(-2, -1), mode='forward')
K = mrmax.operators.LinearOperatorMatrix(((acquisition_operator,), (nabla,)))

# %% [markdown]
# #### $g(x) = 0,$
#
# implemented as `~mrmax.operators.functionals.ZeroFunctional`

# %%
g = mrmax.operators.functionals.ZeroFunctional()
# %% [markdown]
# ```{note}
...
# ```{note}
# We can use the `callback` parameter of `~mrmax.algorithms.optimizers` to get some information
...
from mrmax.algorithms.optimizers.pdhg import PDHGStatus
...
# %%
(img_pdhg_24,) = mrmax.algorithms.optimizers.pdhg(
    f=f,
    g=g,
    operator=K,
