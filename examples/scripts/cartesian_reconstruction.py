# %% [markdown]
# # Basics of mrmax and Cartesian reconstructions
# Here, we are going to have a look at a few basics of mrmax and reconstruct data acquired with a Cartesian sampling
# pattern.

# %% [markdown]
# ## Overview
# In this notebook, we are going to explore the `~mrmax.data.KData` object and the included header parameters.
...
#
# To read in an ISMRMRD file, we can simply pass on the file name to a `~mrmax.data.KData` object.
# Additionally, we need to provide information about the trajectory. In mrmax, this is done using trajectory
...
# calculator `~mrmax.data.traj_calculators.KTrajectoryCartesian` without any further parameters.
#
...
kdata = mrmax.data.KData.from_file(
    data_folder / 'cart_t1.mrd',
    mrmax.data.traj_calculators.KTrajectoryCartesian(),
)

# %% [markdown]
# Now we can explore this data object.
# Simply printing ``kdata`` gives us a basic overview of the `~mrmax.data.KData` object.
...
# `~mrmax.operators.FourierOp` or manually set up a Fast Fourier Transform (FFT).
# For demonstration purposes, we first show the manual approach.
#
# ```{note}
#  Most of the time, you will use the `~mrmax.operators.FourierOp` operator, which automatically takes care
# of choosing  whether to use a FFT or a non-uniform FFT (NUFFT) based on the trajectory.
# It optionally can be created from a `~mrmax.data.KData` object without any further information.
# ```
#
# Let's create an FFT-operator `~mrmax.operators.FastFourierOp` and apply it to our `~mrmax.data.KData` object.
# Please note that all mrmax operator work on JAX arrays and not on the mrmax objects directly. Therefore, we have
# to call the operator on kdata.data. One other important property of mrmax operators is that they always return a
# tuple of JAX arrays, even if the output is only a single tensor. This is why we use the ``(img,)`` syntax below.

# %%
fft_op = mrmax.operators.FastFourierOp(dim=(-2, -1))
...
# `~mrmax.operators.FastFourierOp` to crop the image by providing the correct output matrix size ``recon_matrix``.

# %%
# Create FFT-operator with correct output matrix size
fft_op = mrmax.operators.FastFourierOp(
    dim=(-2, -1),
...
# coils using root-sum-of-squares and then display the image. Note that we usually index from behind in mrmax
...
def show_images(*images: jnp.ndarray, titles: list[str] | None = None) -> None:
    """Plot images."""
    n_images = len(images)
...
magnitude_fully_sampled = jnp.sqrt(jnp.sum(jnp.abs(img) ** 2, axis=-4)).squeeze()
...
# %% tags=["remove-output"]
# Read in the data
kdata_pe_pf = mrmax.data.KData.from_file(
...
magnitude_pe_pf = jnp.sqrt(jnp.sum(jnp.abs(img_pe_pf) ** 2, axis=-4)).squeeze()
...
# ```{note}
# This is because mrmax saves meta data such as trajectories in an efficient way, where dimensions in which the data
# does not change are often collapsed. The original shape can be obtained by
# [broadcasting](https://numpy.org/doc/stable/user/basics.broadcasting.html).
# ```
# To get the full trajectory as a tensor, we can also just call `~mrmax.data.KTrajectory.as_tensor()`:
...
# FFT-operator to, we have got the `~mrmax.operators.CartesianSamplingOp` in mrmax. This operator performs
# sorting based on the k-space trajectory and the dimensions of the encoding k-space.
#
# Let's try it out!

# %%
cart_sampling_op = mrmax.operators.CartesianSamplingOp(
...
# In mrmax, we have a smart `~mrmax.operators.FourierOp` operator, that automatically does the resorting and can
# handle non-cartesian data as well. For cartesian data, it internally does exactly the two steps we just did manually.
# The operator can be also be created from an existing `~mrmax.data.KData` object
# This is the recommended way to transform k-space data.

# %%
fourier_op = mrmax.operators.FourierOp.from_kdata(kdata_pe_pf)
...
# from different coils. In mrmax, you can do this by calculating coil sensitivity data and then creating a
# `~mrmax.operators.SensitivityOp` to combine the data after image reconstruction.
...
csm_data = mrmax.algorithms.csm.walsh(img_pe_pf[0, ...], smoothing_width=5)[None, ...]

# Create SensitivityOp
csm_op = mrmax.operators.SensitivityOp(csm_data)
...
# was a bit cumbersome. To make our life easier, mrmax allows to combine the operators first, get the adjoint
...
#
# That is why we also included some top-level reconstruction algorithms in mrmax. For this whole steps from above,
# we can simply use a `~mrmax.algorithms.reconstruction.DirectReconstruction`.
# Reconstruction algorithms can be instantiated from only the information in the `~mrmax.data.KData` object.
#
# In contrast to operators, top-level reconstruction algorithms operate on the data objects of mrmax, i.e. the input is
# a `~mrmax.data.KData` object and the output is an `~mrmax.data.IData` object containing
# the reconstructed image data. To get its magnitude, we can call the `~mrmax.data.IData.rss` method.

# %%

# Create DirectReconstruction object from KData object
direct_recon_pe_pf = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_pe_pf)
...
# %%
kdata_us = mrmax.data.KData.from_file(
    data_folder / 'cart_t1_msense_integrated.mrd',
    mrmax.data.traj_calculators.KTrajectoryCartesian(),
)
direct_recon_us = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_us)
...
# `~mrmax.data.KData.from_file`.
# However, we can change the filter criterion to `is_coil_calibration_acquisition` to read in only these acquisitions.
#
# ```{note}
# There are already some other filter criteria available, see `mrmax.data.acq_filters`. You can also implement your own
# function returning whether to include an acquisition
# ```
#
# %%
kdata_calib_lines = mrmax.data.KData.from_file(
    data_folder / 'cart_t1_msense_integrated.mrd',
    mrmax.data.traj_calculators.KTrajectoryCartesian(),
    acquisition_filter_criterion=mrmax.data.acq_filters.is_coil_calibration_acquisition,
)

direct_recon_calib_lines = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_calib_lines)
...
# %% [markdown]
# ### Reconstruction
# We can now use these CSMs in a new `~mrmax.algorithms.reconstruction.DirectReconstruction`:

# %%
direct_recon_us_csm = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_us, csm=direct_recon_calib_lines.csm)
...
# As you might have guessed, these are also included in mrmax:
# Instead of the `~mrmax.algorithms.reconstruction.DirectReconstruction`,
# we can use `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction`:

# %%
sense_recon_us = mrmax.algorithms.reconstruction.IterativeSENSEReconstruction(
    kdata_us,
...
# %% [markdown]
# This looks better! More information about the iterative SENSE reconstruction and its implementation in mrmax
