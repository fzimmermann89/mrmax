...
kdata = mrmax.data.KData.from_file(data_folder / '2D_GRad_map_t1.h5', mrmax.data.traj_calculators.KTrajectoryIsmrmrd())
...
idx_dynamic = mrmax.utils.split_idx(kdata.header.acq_info.acquisition_time_stamp.squeeze().argsort(), 30, 0)
...
reconstruction_dynamic = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_dynamic, csm=reconstruction.csm)
...
# `~mrmax.operators.models.TransientSteadyStateWithPreparation`-model and does not have to be added here.
# ```{note}
...
# Therefore, we have to use the parameter `~mrmax.data.KHeader.echo_spacing`, which describes the time between
# two gradient echoes.
...
# %%
model_op = mrmax.operators.models.TransientSteadyStateWithPreparation(
...
# `~mrmax.operators.MagnitudeOp`.

# %%
magnitude_model_op = mrmax.operators.MagnitudeOp() @ model_op
...
# of 50%.
# We use a `~mrmax.operators.ConstraintsOp` to define these constraints. It maps unconstrained parameters to constrained
...
constraints_op = mrmax.operators.ConstraintsOp(
    bounds=(
...
# model $q$.
# %%
mse_loss = mrmax.operators.functionals.MSE(img_rss_dynamic)
...
# %% [markdown]
# If we use a `~mrmax.operators.ConstraintsOp`, the start values must be transformed to the
...
# %%
result = mrmax.algorithms.optimizers.lbfgs(functional, initial_parameters=initial_parameters)
# %% [markdown]
...
# Try to exchange `~mrmax.algorithms.reconstruction.DirectReconstruction` above with
# `~mrmax.algorithms.reconstruction.IterativeSENSEReconstruction`
# or try a different optimizer such as `~mrmax.algorithms.optimizers.adam`.
