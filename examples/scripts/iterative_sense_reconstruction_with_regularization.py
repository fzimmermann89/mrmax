...
# Here we use the `~mrmax.algorithms.reconstruction.RegularizedIterativeSENSEReconstruction` class to reconstruct
...
# We use the `~mrmax.algorithms.reconstruction.RegularizedIterativeSENSEReconstruction` class to reconstruct images
...
kdata_fullysampled = mrmax.data.KData.from_file(
    data_folder / 'radial2D_402spokes_golden_angle_with_traj.h5',
    mrmax.data.traj_calculators.KTrajectoryIsmrmrd(),
)
kdata_undersampled = mrmax.data.KData.from_file(
...
direct_reconstruction = mrmax.algorithms.reconstruction.DirectReconstruction(kdata_fullysampled)
...
regularized_iterative_sense_reconstruction = mrmax.algorithms.reconstruction.RegularizedIterativeSENSEReconstruction(
    kdata_undersampled,
    csm=csm,
    n_iterations=6,
...
# perform them manually. This also demonstrates how to use the `~mrmax` operators and algorithms
...
# %%
dcf_operator = mrmax.data.DcfData.from_traj_voronoi(kdata_undersampled.traj).as_operator()
fourier_operator = mrmax.operators.FourierOp.from_kdata(kdata_undersampled)
...
# We define $H= A^H W A + l$. We use the `~mrmax.operators.IdentityOp` and make
# use of operator composition using ``@``, addition using ``+`` and multiplication using ``*``.
# The resulting operator is a `~mrmax.operators.LinearOperator` object.

# %%
operator = (
    acquisition_operator.H @ dcf_operator @ acquisition_operator + mrmax.operators.IdentityOp() * regularization_weight
)
...
# obtained using `~mrmax.algorithms.reconstruction.RegularizedIterativeSENSEReconstruction`.
